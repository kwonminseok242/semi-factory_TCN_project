...
def generate_windows_from_sequences(df, sequences, window_size=50, stride=1):
    ...
    for start, end, _, _, _ in sequences:
        ...
        for i in range(0, len(sub_df) - window_size + 1, stride):
...
def save_sliding_window_data(X, y):
    ...
    np.save(os.path.join(save_dir, "X1_windows.npy"), X)
    np.save(os.path.join(save_dir, "y1_labels.npy"), y)
...
if __name__ == "__main__":
    ...
    X, y = generate_windows_from_sequences(df, sequences, window_size=50,stride=1) # stride = 1이 정확도 면에서 더 좋음
